...
merge_splits() {
	...
	if ! OP=$(java -jar "$APKSIGNER" sign --ks ks-p12.keystore --ks-pass "pass:$KEYSTORE_PASSWORD" --key-pass "pass:$KEYSTORE_PASSWORD" --ks-key-alias jhc \
...
--keystore-entry-password=$KEYSTORE_PASSWORD --keystore-password=$KEYSTORE_PASSWORD --signer=jhc --keystore-entry-alias=jhc -t '$tmp_files' $patcher_args"
